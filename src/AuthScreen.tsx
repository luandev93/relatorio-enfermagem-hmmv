...
export const AuthScreen: React.FC<AuthScreenProps> = ({ users, onLogin }) => {
  ...
  const handleUserLogin = (e: React.FormEvent) => {
    ...
    // Direct match for admin username (apenas pelo nome de usuário "admin";
    // '0101' foi removido como atalho pois colidia com o login de
    // profissionais sem data de nascimento cadastrada)
    if (trimmedLogin.toLowerCase() === 'admin') {
      const adminUser =
    ...
    const user = users.find((u) => {
      const ddmm = getDDMMFromBirthDate(u.birthDate);
      return (
        u.username?.toLowerCase() === trimmedLogin.toLowerCase() ||
        (ddmm !== '' && ddmm === trimmedLogin)
      );
    });
    ...
    if (!validUserPin) {
      setErrorMsg('Este usuário não possui PIN configurado. Contate o Administrador.');
      return;
    }
    if (pinInput !== validUserPin) {
